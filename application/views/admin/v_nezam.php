...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
                    ...
                    <form id="category-form">
                        <div id="category-add" style="padding:10px;border-top:solid 1px #999">
                            ...
                            <select id="cat-parent" name="parent" class="input full-w medium">
                                ...
                                <?php echo $this->nezam->getNezamSelectMenu($owner); ?>
                            </select>
                            ...
                            <div>
                                ...
                                <input type="button" id="update-category-btn" class="w-btn" value="به روز رسانی"
                                       onClick="updateNezam(this)" style="display:none">

                                <input type="button" id="add-category-btn" class="w-btn" value="جدید"
                                       onClick="addNezam(this)">
	...
	<li item-id="[ID]" pos="[POS]" parent="[PARENT]" special="[SPECIALVAL]" published="[PUBLISHEDVAL]" class="cat-item">
		<div class="cat-item-div">
			...
			<span class="cat-options">
				<i class="fa fa-times red cu" onClick="deleteNezam([ID],this)"></i>
	...
	';
                ...
                echo $this->nezam->getNezamList($owner, 0, FALSE, NULL, FALSE, $sample);
                echo "</div>";
...
<script type="text/javascript">
    ...
    function newLi(cat) {
        ...
        var $li = $('<li/>', {'item-id': id, class: cat.class || 'cat-item'}).attr({
            'parent': par,
            'pos': pos,
            ...
            .append(
                $('<div/>', {class: 'cat-item-div'})
                    ...
                    $('<span/>', {class: 'cat-options'})
                        .append(
                            $('<i/>', {class: 'fa fa-times red cu'}).attr('onclick', 'deleteNezam(' + id + ',this)')
                        ).append(
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.done) {
                    ...
                    dialog_box('افزودن نظام جدید با مشکل مواجه شد');
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.done) {
                    ...
                    $('#cat-parent option:not(:first)').remove();
                    $('#cat-parent').append(cat.menu).val(cat.parent);
                    sortNezam(cat.parent);
                    $('#reset-btn').trigger("click");
                } else {
                    dialog_box('ویرایش نظام جدید با مشکل مواجه شد');
                    $(btn).parent().removeClass('l h6 blue');
                }
                $(btn).parent().removeClass('l h6 blue');
            },
    ...
    function deleteNezam(id, btn) {

        var li = $(btn).closest('li'),
            ul = $(li).find('ul');
        var text = ul.length ? " این نظام دارای زیرمجموعه است . <br/>" : " ";
        text += 'نظام حذف شود ؟'
        var options = {
            url: 'nezam/delete',
            data: {id: id},
