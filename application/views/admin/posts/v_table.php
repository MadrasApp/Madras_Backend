...
<?php
	...
	if($type == 'book')
	{
	    ...
	    if(isset($user_can_edit) && $user_can_edit){
			...
			$cols['دارای اشتراک'] = array(
				'field_name'=>'has_membership',
                'link'=>true,
				'count'=>'has_membership',
				'th-attr' => 'style="width:50px"',
				'function'=>function($col,$row){
					global $extraData;
					$count = $row["has_membership"];
					$html = '<i class="fa fa-lg '.($count?'fa-check text-success':'fa-close text-muted').'"></i>';
					return $html;
				}
			);
		$cols['دانلود'] = array(
		...
		$cols['اشتراک خرید شده'] = array(
			'field_name'=>'has_bought',
            'link'=>true,
			'count'=>'has_bought',
			'th-attr' => 'style="width:50px"',
			//'text-success', 'text-muted'
			'function'=>function($col,$row){
				global $extraData;
				$count = $row["has_bought"];
				$html = '<div style="padding:10px;cursor: pointer" onclick="showMembership(this,'.$row["id"].','.$count.')"><i class="fa fa-lg fa-share-alt '.($count?'text-success':'text-muted').'"></i><br />'.$count.'</div>';
				return $html;
			}
		);
	}
...
<div class="hidden">
    ...
    <div class="view-membership"><!-- Alireza Balvardi -->
        <center>
            <h2>لیست اشتراک کتاب <span class="displaynametag"></span></h2>
        </center>
        <table class="table">
            <thead>
            <tr>
...
//=============================================
    ...
    function showMembership(btn,id){
        var $html = $('<div/>');
        $html.append('<div class="text-center"><i class="l c-c blue h3"></i></div>');
        popupScreen($html);
        $.ajax({
            type: "POST",
            url: 'admin/api/getBookMembership/' + id,
            dataType: "json",
            success: function (data) {
                ...
                var $view = $('.view-membership').clone(true);
                $view.find('.displaynametag').html($('#book_'+id).html());
                for(i=0;i < data.result.length;i++){
                    el = data.result[i];
                    ...
                    html =
                        ...
                        '<th>'+tel+'</th>'+
                        '<th>'+expdate+'</th>'+
                        '<th><i class="fa '+(state=="1"?'fa-check-circle-o text-success':'fa-ban text-danger')+' fa-lg"></i></th>'+
                        '</tr>';
                    html = html.replace(/null/g,'');
                    $view.find('.mobilebody').append(html);
                }
