...
<div class="btn btn-success text-center col-sm-12">
    <div class="h2">دسته بندی موضوعی</div>
</div>
...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
    <tr>
                    ...
                    <form id="category-form">
                        <div id="category-add" style="padding:10px;border-top:solid 1px #999">
                            ...
                            <select id="cat-parent" name="parent" class="input full-w medium">
                                ...
                                <?php echo $this->mecat->getMeCatSelectMenu($owner); ?>
                            </select>
                            <div>انتشار</div>
                            ...
                            <input id="cat-val" name="name" type="text" class="input full-w" placeholder="عنوان دسته"
                                   title="عنوان دسته" onFocus="$(this).select()">
                            ...
                            <div>
                                ...
                                <input type="button" id="update-category-btn" class="w-btn" value="به روز رسانی"
                                       onClick="updateMeCat(this)" style="display:none">

                                <input type="button" id="add-category-btn" class="w-btn" value="جدید"
                                       onClick="addMeCat(this)">
        ...
        <td width="70%" style="vertical-align:top;width:70%">
            <div class="category-table" style="margin-top:15px">
                ...
                $sample =
                    '
	<li item-id="[ID]" pos="[POS]" parent="[PARENT]" special="[SPECIALVAL]" published="[PUBLISHEDVAL]" class="cat-item">
		<div class="cat-item-div">
			...
			<span class="cat-options">
				<i class="fa fa-times red cu" onClick="deleteMeCat([ID],this)"></i>
	...
	';
                ...
                echo $this->mecat->getMeCatList($owner, 0, FALSE, NULL, FALSE, $sample);
                echo "</div>";
...
<script type="text/javascript">
    ...
    function newLi(cat) {
        ...
        var $li = $('<li/>', {'item-id': id, class: cat.class || 'cat-item'}).attr({'parent': par, 'pos': pos,'special':cat.special,'published':cat.published})
            .append(
                $('<div/>', {class: 'cat-item-div'})
                    .append(
                    ...
                    $('<span/>', {class: 'cat-options'})
                        .append(
                            $('<i/>', {class: 'fa fa-times red cu'}).attr('onclick', 'deleteMeCat(' + id + ',this)')
                        ).append(
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.done) {
                    ...
                    dialog_box('افزودن دسته بندی موضوعی جدید با مشکل مواجه شد');
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.done) {
                    ...
                    $('#cat-parent option:not(:first)').remove();
                    $('#cat-parent').append(cat.menu).val(cat.parent);
                    sortMeCat(cat.parent);
                    $('#reset-btn').trigger("click");
                } else {
                    dialog_box('ویرایش دسته بندی موضوعی جدید با مشکل مواجه شد');
                    $(btn).parent().removeClass('l h6 blue');
                }
                $(btn).parent().removeClass('l h6 blue');
            },
    ...
    function deleteMeCat(id, btn) {

        var li = $(btn).closest('li'),
            ul = $(li).find('ul');
        var text = ul.length ? " این دسته بندی موضوعی دارای زیرمجموعه است . <br/>" : " ";
        text += 'دسته بندی موضوعی حذف شود ؟'
        var options = {
            url: 'mecat/delete',
            data: {id: id},
            success: function (data) {
