...
class Suppliertype extends CI_Controller {
    ...
    function __construct(){
        ...
        parent::__construct();
        $this->load->model('m_suppliertype','suppliertype');
        ...
        {
            $this->user->checkAccess('manage_suppliertype');
        }
    }
    ...
    public function index($action='all')
    {
        ...
        $data['_title'] = ' | Suppliertype';

        $data['_tabs']['all'] = array('name'=>'همه','count'=>$this->db->count_all_results('suppliertype'));

        $data['_tabs']['pending'] = array('name'=>'تایید نشده','count'=>$this->db->count_all_results('suppliertype'));

        $data['_tabs']['status'] = array('name'=>'تایید شده','count'=>$this->db->count_all_results('suppliertype'));
        ...
        $searchQuery        = $this->tools->createSearchQuery($fields,'suppliertype');

        $data['query'] = " WHERE 1 $searchQuery ";

        if( $this->user->can('delete_comment') )
            $data['options'][] = array('name'=>'حذف','icon'=>'trash-o','click'=>'delete_row(this,\'suppliertype\',[FLD])');

        $data['tableName'] = 'suppliertype';
        ...
        $this->_view('v_suppliertype',$data);
    }
