...
class Salereport extends CI_Controller {
	...
	function __construct(){
		...
		{
			$this->user->checkAccess('manage_salereport');
		}			
	}
	...
	public function index()
	{
		...
		if($level != "admin" && $this->user->can('is_supplier')){
			...
			if($row){
				$title = "اطلاعات فروش محصولات $row->title";
				$ownerpercent = $row->ownerpercent;
			}
		}

		$data['_title'] = ' | گزارش فروش';
		...
		********************************/
        ...
        $query  = "where d.discount > 0 $searchQuery ";
        ...
        $query .= " GROUP BY d.book_id order by ";
        $order = $this->input->get($tableName.'_order');
        $query .= ($order ? $order:'p.title')." ";
