...
class Diclang extends CI_Controller {

    function __construct(){

        parent::__construct();
        $this->load->model('m_diclang','diclang');
        ...
        {
            $this->user->checkAccess('manage_diclang');
        }
    }
    ...
    public function index($action='all')
    {
        ...
        $data['_tabs']['all'] = array('name'=>'همه','count'=>$this->db->count_all_results('diclang'));

        $data['_tabs']['pending'] = array('name'=>'تایید نشده','count'=>$this->db->count_all_results('diclang'));

        $data['_tabs']['status'] = array('name'=>'تایید شده','count'=>$this->db->count_all_results('diclang'));
        ...
        $searchQuery        = $this->tools->createSearchQuery($fields,'diclang');

        $data['query'] = " WHERE 1 $searchQuery ";

        if( $this->user->can('delete_comment') )
            $data['options'][] = array('name'=>'حذف','icon'=>'trash-o','click'=>'delete_row(this,\'diclang\',[FLD])');

        $data['tableName'] = 'diclang';
