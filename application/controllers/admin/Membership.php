...
class Membership extends CI_Controller {
    ...
    function __construct(){
        ...
        {
            $this->user->checkAccess('manage_membership');
        }
    }
    ...
    public function index($action='all')
    {
        ...
        $data['_tabs']['all'] = array('name'=>'همه','count'=>$this->db->count_all_results('membership'));

        $data['_tabs']['pending'] = array('name'=>'تایید نشده','count'=>$this->db->count_all_results('membership'));

        $data['_tabs']['status'] = array('name'=>'تایید شده','count'=>$this->db->count_all_results('membership'));
        ...
        $searchQuery        = $this->tools->createSearchQuery($fields,'membership');

        $data['query'] = " WHERE 1 $searchQuery ";

        if( $this->user->can('delete_comment') )
            $data['options'][] = array('name'=>'حذف','icon'=>'trash-o','click'=>'delete_row(this,\'membership\',[FLD])');

        $data['tableName'] = 'membership';
