...
class Ostad extends CI_Controller {

    function __construct(){

        parent::__construct();
        $this->load->model('m_ostad','ostad');
        ...
        {
            $this->user->checkAccess('manage_ostad');
        }
    }
    ...
    public function index($action='all')
    {
        ...
        $data['_tabs']['all'] = array('name'=>'همه','count'=>$this->db->count_all_results('ostad'));
        ...
        $searchQuery        = $this->tools->createSearchQuery($fields,'ostad');

        $data['query'] = " WHERE 1 $searchQuery ";

        if( $this->user->can('delete_comment') )
            $data['options'][] = array('name'=>'حذف','icon'=>'trash-o','click'=>'delete_row(this,\'ostad\',[FLD])');

        $data['tableName'] = 'ostad';
