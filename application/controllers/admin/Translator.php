...
class Translator extends CI_Controller {
    ...
    function __construct(){
        ...
        parent::__construct();
        $this->load->model('m_translator','translator');
        ...
        {
            $this->user->checkAccess('manage_translator');
        }
    }
    ...
    public function index($action='all')
    {
        ...
        $data['_title'] = ' | Translator';

        $data['_tabs']['all'] = array('name'=>'همه','count'=>$this->db->count_all_results('translator'));
        ...
        $searchQuery        = $this->tools->createSearchQuery($fields,'translator');

        $data['query'] = " WHERE 1 $searchQuery ";

        if( $this->user->can('delete_comment') )
            $data['options'][] = array('name'=>'حذف','icon'=>'trash-o','click'=>'delete_row(this,\'translator\',[FLD])');

        $data['tableName'] = 'translator';
