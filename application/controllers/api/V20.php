...
class V2 extends CI_Controller {
	...
	public function getKalameh(){
		...
		$Limitstart = (int)$data["Limitstart"];
...
*/
	...
	public function allbookList(){
		...
		$this->db->select('p.id,p.title,p.excerpt description,p.category parentcategoryid,p.category parentcategoryname,p.category categoryid,p.category categoryname,p.thumb,p.price');
		if($catid)
		...
		$post_meta = $this->db->select('*')->where('post_id IN('.implode(',',$book_ids).')')
	...
	public function getBook($id=NULL){
		$user    = $this->_loginNeed(TRUE, 'u.id');
		if($userid){
			$user = $this->db->where('tel',$userid)->get('users')->row();
		}
//		if($user === FALSE) throw new Exception("برای دسترسی به این بخش باید وارد حساب کاربری خود شوید" , -1);
		...
		$type=$this->input->post('type')?$this->input->post('type'):'zip';
		$filename = md5($id);
...
/* Letner */
	...
	public function getLastClasses(){
		...
		$section = $this->input->post('section');
		$baseurl = base_url();
		$allowsection = array('doreh','classroom','favclass','dorehclass','ostad','place','book','favbook','bookupdate','supplierbook','supplierfavbook','specialbook','samembook','sametbook');
	...
	//=========================================
	public function getClassJalasatDetail($id){
			$base = base_url();
	...
	//=========================================
	public function getClassDetail($id=0,$return = 0,$group = 1,$subjalasat=0){
		try
		{
			$id = $id?$id:(int)$this->input->post('id');
			if( ! $id )
...
/*
			$dorehclass = $this->db
				->select("dc.*,c.title,t.name dorehtitle,CONCAT(d.tahsili_year,'-',d.tahsili_year+1) tahsili_year,s1.title ostadtitle,s2.title placetitle")
				->where('dc.id',$id)
				...
				->join('ci_supplier s1','s1.id=dc.ostadid','INNER',FALSE)
				->join('ci_supplier s2','s2.id=dc.placeid','INNER',FALSE)
				->get('dorehclass dc')->row();
*/
			...
			$jalasats = array();
			$db = $this->db;
            $db->select('j.*,d.audio,d.bookid,CEIL(d.audio_duration) duration,d.pages')
				->where('j.dorehclassid',$id)
				->where('j.published = 1')
			...
			if($subjalasat){
	            $db->select('sj.bookid');
				$db->join('ci_subjalasat sj','d.jid=sj.jalasatid','INNER',FALSE);
	            //$db->select('sj.id sjd,sj.description sjdescription,sj.duration,sj.startPos,sj.endPos');
	            //$db->group_by('sj.id');
			}
			...
			foreach($O as $k=>$jalasat){
				...
				foreach($pages as $kp=>$order){
					...
					if($subjalasat){
						$OSJ = $db->order_by('s.')->where('s.jalasatid',$jalasat->id)->get('subjalasat s')->result();
						$jalasat->subjalasat = $OSJ;
					} else {
	...
	public function getBookDetail(){
        try
        {
			...
			*/
            ...
            $this->tools->outS(0,'OK',array('book'=>$book,"classroom_data"=>$classroom_data,"dorehclass"=>$dorehclass,"same_mozoe"=>$same_mozoe,"same_onvan"=>$same_onvan));
        }
        catch(Exception $e)
        {
            $this->tools->outE($e);
	...
	public function userClasses(){
        try
        {
            ...
            $O = $this->db
				->select("u.*")
				->join('ci_dorehclass d','u.dorehclassid=d.id','INNER',false)
				->where("u.factor_id IN(".implode(",",$factors).")")
				->where("d.jalasat > 0")
				->get('user_doreh u')->result();
			foreach($O as $k=>$v){
				$data = self::getClassDetail($v->dorehclassid,1,1,1);
				if(count($data))
				$dorehclassid[] = $data;
			}
