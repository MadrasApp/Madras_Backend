...
*/
	...
	public function getBook0($id=NULL,$type='zip'){
		$user    = $this->_loginNeed(TRUE, 'u.id');
    ...
    public function getBook($id = NULL, $type = 'zip')
    {
        ...
        $exist = $this->db->where('id', $id)->where('type', 'book')->get('posts')->result();

        if (count($exist) == 0) {
            throw new Exception("Invalid book id", 2);
        }
        if ($exist[0]->published == 0) {
            throw new Exception("This book is not active", 1);
        }

        $dir = "temp/book";
        $temp = "$dir/$filename.$type";
        if (!is_dir($dir)) {
            mkdir($dir);
        }
        if (0 && file_exists($temp)){
            if($type!="json")
        ...
        $this->load->model('m_book', 'book');
        //$this->Pre(["A"=>microtime(true) - $time],0);
        //get the book info
        $book = $this->post->getPosts([
            'user_id' => $userid,
            'type' => 'book',
        ...
        //$this->Pre(["B"=>microtime(true) - $time],0);

        $this->db->select('c.id AS value,c.displayname as text');
        $this->db->where_in('c.level', 'teacher');
        $teachers = $this->db->get('users c')->result();
        //$this->Pre(["C"=>microtime(true) - $time],0);
        $tempteachers = [];
        foreach ($teachers as $teacher) {
            $tempteachers[$teacher->value] = $teacher;
        }

        //$this->Pre(["D"=>microtime(true) - $time],0);
        ...
        $classonlines = $this->db->select('cid')
            ...
            ->result();
        //$this->Pre(["E"=>microtime(true) - $time],0);
        $classonline_ids = [0];
        ...
        $classonlines = $this->db->select('*')
            ...
            ->get('classonline_data')
            ->result();
        //$this->Pre(["F"=>microtime(true) - $time],0);
        $dayofweeks = [
            0 => "شنبه",
            1 => "یکشنبه",
            2 => "دوشنبه",
        ...
        //$this->Pre(["G"=>microtime(true) - $time],0);

        $classonlines = $this->db->select('*')
            ->where_in('id', $classonline_ids)
            ->get('classonline')
            ->result();
        //$this->Pre(["H"=>microtime(true) - $time],0);

        foreach ($classonlines as $key => $classonline) {
            if ($classonline->teachername) {
                $classonlines[$key]->teachername = $tempteachers[$classonline->teachername];
            }
        ...
        $classrooms = $this->db->select('*')
            ...
            ->get('classroom_data')
            ->result();
        //$this->Pre(["I"=>microtime(true) - $time],0);
        ...
        $classrooms = $this->db->select('*')
            ...
            ->get('classroom')
            ->result();
        //$this->Pre(["J"=>microtime(true) - $time],0);
        ...
        //$this->Pre(["K"=>microtime(true) - $time]);
        ...
        if ($type == 'json') {
            ...
            $data = json_encode($result,JSON_UNESCAPED_UNICODE | JSON_HEX_QUOT | JSON_HEX_TAG);
            //file_put_contents($temp,(string)$data);
	...
	public function getSupplier($id=0){
		try
		{
			...
			if( ! $this->db->where('id',$id)->count_all_results('supplier') )
				throw new Exception( 'مردی یافت نشد', 2);
