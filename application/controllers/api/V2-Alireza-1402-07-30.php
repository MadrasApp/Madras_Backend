    ...
    public function getBook($id = NULL, $type = 'zip')
    {
        ...
        foreach ($classonlines as $classonline) {
            if ($classonline->data_type == "dayofweek") {
                $classonline_dayofweeks[$classonline->cid][] = ["dayofweek" => $classonline->dayofweek, "starttime" => $classonline->starttime, "endtime" => $classonline->endtime];
            }
        }
    ...
    public function getBookDetail()
    {
        try {
			...
			*/
            ...
            $this->tools->outS(0, 'OK', array('book' => $book,"dayofweek"=>$dayofweek, "classroom_data" => $classroom_data, "dorehclass" => $dorehclass, "same_mozoe" => $same_mozoe, "same_onvan" => $same_onvan, "membership" => $membership));
        } catch (Exception $e) {
            $this->tools->outE($e);
        }
    }
