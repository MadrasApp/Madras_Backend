                ...
                foreach ($attrs['elements'] as $element => $eParts) {
                    if (isset($eParts['ref'])) {
                        ...
                        foreach ($eParts as $aName => $aValue) {
                            // handle, e.g., abstract, default, form, minOccurs, maxOccurs, nillabel
...
class wsdl extends nusoap_base
{
    ...
    function serializeType($name, $type, $value, $use = 'encoded', $encodingStyle = FALSE, $unqualified = FALSE)
    {
            ...
            if ($ns == $this->XMLSchemaVersion || $ns == 'http://schemas.xmlsoap.org/soap/encoding/') {
                ...
                if (is_null($value)) {
                    if ($use == 'literal') {
                        ...
                        // TODO: depends on nillabel, which should be checked before calling this method
        ...
        if ($phpType == 'struct') {
            ...
            if (is_null($value)) {
                if ($use == 'literal') {
                    // TODO: depends on minOccurs and nillabel
    ...
    function serializeComplexTypeElements($typeDef, $value, $ns, $uqType, $use = 'encoded', $encodingStyle = FALSE)
    {
        ...
        if (isset($typeDef['elements']) && is_array($typeDef['elements'])) {
            ...
            foreach ($typeDef['elements'] as $eName => $attrs) {
                ...
                if (isset($optionals)
                    && (!isset($xvalue[$eName]))
                    && ((!isset($attrs['nillabel'])) || $attrs['nillabel'] != 'true')
                ) {
                    if (isset($attrs['minOccurs']) && $attrs['minOccurs'] <> '0') {
                        $this->debug("apparent error: no value provided for element $eName with minOccurs=" . $attrs['minOccurs']);
                    }
                    // do nothing
                    $this->debug("no value provided for complexType element $eName and element is not nillabel, so serialize nothing");
                } else {
                    // get value
                    if (isset($xvalue[$eName])) {
                        $v = $xvalue[$eName];
                    ...
                    if (isset($attrs['maxOccurs']) && ($attrs['maxOccurs'] == 'unbounded' || $attrs['maxOccurs'] > 1) && isset($v) && is_array($v) && $this->isArraySimpleOrStruct($v) == 'arraySimple') {
                        ...
                        if (is_null($v) && isset($attrs['minOccurs']) && $attrs['minOccurs'] == '0') {
                            // do nothing
                        } elseif (is_null($v) && isset($attrs['nillabel']) && $attrs['nillabel'] == 'true') {
